...
## 当前入口：8th Wall 小吊牌 AR（2026-09-20）

引擎继续使用原项目的 **8th Wall / XR8 1.0.0 + Three.js 0.160.1**。此前改为 MindAR、把扫码入口跳转至非相机动画页的决定已撤销。`/v3/` 和首页进入 `/ar/`，后者打开相机管线；`/animation/` 仅为内部资产预览，不是消费者入口。

本地已接入五角色连续浅浮雕：原图、比例、曲面与动态法线保留，透明度0.75；识别后每1000ms硬切，丢失立即清除计时并隐藏。跟踪适配器负责物理尺度换算，内容容器固定在米制坐标 `(0,0,0.015)`。

**本次修复：** 相机画布由 XR8 管理尺寸，不再由 Three.js 的 DPR 设置覆盖成左上角小窗口。定位数据只加载 `tie-label`：从既有印刷母版裁取实物吊牌仍完整保留的矩形区域，不用旧整卡定位、不改二维码、不要求重印。

**验证边界：** 已使用用户授权的试管照片，在真实 XR8 管线中检出 `tie-label` 并播放原有五角色；检查了全屏、硬切循环、丢失隐藏与重新检出。照片回放不等于真机距离、反光、手持稳定性验收。尺寸换算仍以母版的名义印刷尺度为依据，不冒充实测。详见 [回放证据](tests/tie-replay-verification.md)。部署以 GitHub Pages 工作流结果为准。

本轮只需看：`site/ar/` 是扫码相机体验；`site/ar/image-targets/tie-label.png` 是机器定位用的既有印刷裁片，不是新打印物；`tests/tie-replay.js` 是仅本地回放，不发布、不采集真实摄像头。

以下为历史记录，涉及 MindAR、非相机扫码入口的陈述均已作废。
